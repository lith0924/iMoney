...
public class TenantController extends BladeController {
	...
	public R<IPage<Tenant>> list(@ApiIgnore @RequestParam Map<String, Object> tenant, Query query, BladeUser bladeUser) {
		...
		IPage<Tenant> pages = tenantService.page(Condition.getPage(query), (!bladeUser.getTenantId().equals("000000")) ? queryWrapper.lambda().eq(Tenant::getTenantId, bladeUser.getTenantId()) : queryWrapper);
		return R.data(pages);
	}
	...
	public R<List<Tenant>> select(Tenant tenant, BladeUser bladeUser) {
		...
		List<Tenant> list = tenantService.list((!bladeUser.getTenantId().equals("000000")) ? queryWrapper.lambda().eq(Tenant::getTenantId, bladeUser.getTenantId()) : queryWrapper);
		return R.data(list);
	}
