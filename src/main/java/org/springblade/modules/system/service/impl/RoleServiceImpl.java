	...
	public List<RoleVO> tree(String tenantId) {
		...
		if (!userRole.equals("administrator")) {
			Role role = this.getOne(new LambdaQueryWrapper<Role>()
				.eq(Role::getRoleAlias, userRole));
			this.getChildIdsRecursive(role.getId(), roleIds);

		}
		return ForestNodeMerger.merge(baseMapper.tree(tenantId, excludeRole, roleIds));
	}

	@Override
	public void getChildIdsRecursive(Long parentId, List<Long> result) {
		// 查询直接子 id
		LambdaQueryWrapper<Role> queryWrapper = new LambdaQueryWrapper<>();
		queryWrapper.eq(Role::getParentId, parentId)
			.eq(Role::getIsDeleted, 0);  // 排除已删除的记录
		List<Role> children = baseMapper.selectList(queryWrapper);

		// 递归查询子 id 的子 id
		for (Role child : children) {
			result.add(child.getId());
			getChildIdsRecursive(child.getId(), result);
		}
	}
